...
export default class TodoApp extends Component {
    ...
    componentDidMount = async() => {
        ...
        const todos = await request.get(`https://tranquil-temple-40759.herokuapp.com/api/todos`)
    ...
    handleClick = async () => {
        ...
        const data = await request.post(`https://tranquil-temple-40759.herokuapp.com/api/todos`, {
            task: this.state.todoInput
        })
            .set('Authorization', user.token);
    }
            ...
            />
                {
                    this.state.todos.map((todo) => <p 
                        ...
                        this.setState({ todos: newTodos });
                        const data = await request.put(`https://tranquil-temple-40759.herokuapp.com/api/todos/${todo.id}`, matchingTodo)
                        .set('Authorization', user.token);
                    }} key={todo.id}>
                        {todo.task}
                    </p>)
